...
// Lazily initialize Upstash Redis client at request time (avoids build-time execution)
...
async function isKVAvailable() {
  try {
    ...
    if (!client) {
      console.log("❌ Redis client not initialized (missing env)")
      return false
    }
    await client.ping()
...
export async function GET() {
  try {
    ...
    if (kvAvailable) {
      ...
      const client = getRedisClient()
      const content = client ? await client.get(CMS_KEY) : null
      ...
      if (client) {
        await client.set(CMS_KEY, defaultContent)
      }
  ...
  try {
    ...
    if (kvAvailable) {
      ...
      try {
        ...
        await client.set(CMS_KEY, content)
